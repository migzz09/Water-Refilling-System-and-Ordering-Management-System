  ...
  try {
    ...
    const items = cart.map(item => {
      ...
      return {
        ...
        water_type_name: item.water_type_name || '',
    ...
    if (selectedPaymentMethod === 'gcash') {
      ...
      const batchCheck = await batchCheckRes.json();
      if (!batchCheck.success) {
        if (typeof showToast === 'function') showToast(batchCheck.message || 'Batch is full or unavailable.', 'error', 6000);
        return;
      }
      // 2. Proceed to PayMongo only if batch is available
      const checkoutData = {
        cart: items,
        order: {
          order_type: 1,
      ...
      console.log('PayMongo Result:', payMongoResult);
      if (payMongoResult.success && payMongoResult.checkout_url) {
        // Open PayMongo in a popup window
        const width = 600;
        const height = 700;
        ...
        const paymentWindow = window.open(
          payMongoResult.checkout_url,
          'PayMongoCheckout',
          `width=${width},height=${height},left=${left},top=${top},resizable=yes,scrollbars=yes`
        );

        if (!paymentWindow) {
          showToast('Please allow popups for this site to complete payment', 'error');
          return;
        }
        ...
        let checkCount = 0;
        const maxChecks = 1200; // 10 minutes (1200 * 500ms)
        ...
        const checkInterval = setInterval(() => {
          ...
          try {
            // Try to check the popup URL
            try {
              const popupUrl = paymentWindow.location.href;
              
              if (popupUrl.includes('/WRSOMS/api/paymongo/payment_success.php')) {
                paymentCompleted = true;
                clearInterval(checkInterval);
                // Redirect parent window to success page
                window.location.href = popupUrl;
                return;
              } else if (popupUrl.includes('/WRSOMS/api/paymongo/payment_failed.php')) {
                paymentCompleted = true;
                clearInterval(checkInterval);
                // Redirect parent window to failure page
                window.location.href = popupUrl;
                return;
              }
            } catch (e) {
              // CORS error is expected when on PayMongo domain - this is normal
            }
            ...
            if (paymentWindow.closed) {
              ...
              if (!paymentCompleted) {
                // User closed the window without completing payment
                console.log('Payment window closed without completion');
                
                // Wait a moment to see if we get redirected (in case popup closed itself after success)
                setTimeout(() => {
            ...
            console.error('Error monitoring payment window:', e);
          }
        }, 500);

        return;
      } else {
        showToast(payMongoResult.message || 'Failed to initiate GCash payment', 'error');
        return;
      }
    }
