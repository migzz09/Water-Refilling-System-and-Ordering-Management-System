...
<body>
    <nav class="sidebar" aria-label="Main navigation">
        ...
        <ul>
            ...
            <li><button type="button" onclick="showManageStatus()">Manage Status</button></li>
    ...
    fetch('status.php')
        ...
        .then(data => {
            const parser = new DOMParser();
            const doc = parser.parseFromString(data, 'text/html');
            const content = doc.querySelector('.container')?.outerHTML || "<p>Unable to load Manage Status.</p>";
            document.getElementById('mainContent').innerHTML = content;
            document.querySelector('.sidebar li.active').classList.remove('active');
            document.querySelector('.sidebar li button[onclick="showManageStatus()"]').parentElement.classList.add('active');
        })
        .catch(error => {
            console.error('Error loading Manage Status:', error);
            document.getElementById('mainContent').innerHTML = "<p>Unable to load Manage Status.</p>";
        });
}
