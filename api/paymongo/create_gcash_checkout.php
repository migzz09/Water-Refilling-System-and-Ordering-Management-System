...
foreach ($cart as $item) {
    ...
    // Build descriptive name including order type for clarity
    $orderTypeName = $item['order_type_name'] ?? '';
    $waterTypeName = $item['water_type_name'] ?? '';
    $displayName = $name ?: 'Item';
    
    // Add order type and water type to the name for better clarity on PayMongo checkout
    if ($orderTypeName) {
        $displayName .= ' - ' . $orderTypeName;
    }
    if ($waterTypeName) {
        $displayName .= ' (' . $waterTypeName . ')';
    }
...
if ($checkout_url) {
    ...
    $_SESSION['pending_payment'] = [
        ...
        'payload' => [
            ...
            // 'skip_business_time_checks' => true, // Removed to enforce batch limit
