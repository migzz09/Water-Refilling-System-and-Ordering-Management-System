...
try {
    ...
    // Debug: Log session contents for troubleshooting staff archiving
    error_log('ARCHIVE SESSION DEBUG: ' . print_r($_SESSION, true));
    
    // Allow both admin and staff users
    $userId = null;
    $userType = null;
    if (isset($_SESSION['is_admin']) && $_SESSION['is_admin'] == 1 && isset($_SESSION['username'])) {
        // Admin user
    ...
    } elseif (isset($_SESSION['staff_id']) && isset($_SESSION['staff_role'])) {
        // Staff user (use staff_id directly from session)
        $stmt = $pdo->prepare("SELECT staff_id FROM staff WHERE staff_id = ? LIMIT 1");
    ...
    if (!$userId) {
        sendJson([
            'success' => false,
            'message' => 'Could not determine admin or staff user ID',
            'debug' => [
...
try {
    ...
    // Only log to archive_log if at least one order was archived
    if ($archivedCount > 0) {
        $stmt = $pdo->prepare("
            INSERT INTO archive_log 
            (archive_type, orders_archived, archived_by, archived_at, date_filter, notes)
            VALUES (?, ?, ?, NOW(), ?, ?)
        ");
        $stmt->execute([
            $archiveType,
