...
// Admin authentication check - allow admin users OR staff users
$isAdmin = isset($_SESSION['is_admin']) && $_SESSION['is_admin'] == 1;
$isStaff = isset($_SESSION['staff_role']) && !empty($_SESSION['staff_role']);

// Debug logging
error_log("=== DASHBOARD AUTH CHECK === is_admin: " . ($isAdmin ? 'YES' : 'NO') . ", staff_role: " . ($_SESSION['staff_role'] ?? 'NOT SET') . ", username: " . ($_SESSION['username'] ?? 'NOT SET'));
...
if (!$isAdmin && !$isStaff) {
    ...
    echo json_encode([
        ...
        'debug' => [
            'is_admin' => $isAdmin,
            'has_staff_role' => $isStaff,
            'staff_role_value' => $_SESSION['staff_role'] ?? null,
...
try {
    ...
    // Deduplicate by reference_id, prefer archived
    $unique = [];
    foreach ($order_history as $order) {
        $ref = $order['reference_id'];
        if (!isset($unique[$ref]) || ($order['source'] === 'archived' && $unique[$ref]['source'] !== 'archived')) {
            $unique[$ref] = $order;
        }
    }
    // Re-index and limit to 40
    $order_history = array_slice(array_values($unique), 0, 40);
