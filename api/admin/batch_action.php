...
if ($isAdmin) {
    ...
    if ($username) {
        ...
        if ($srow && !empty($srow['staff_role'])) {
            ...
            // Drivers/riders are now allowed to perform all actions (pickup and delivery)
            if ($staffRole === 'driver') {
                if (in_array($action, ['start_pickup', 'complete_pickup', 'start_delivery', 'complete_delivery'])) {
                    $isAuthorized = true;
                }
            }
